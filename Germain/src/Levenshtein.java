...
public class Levenshtein {
	...
	public static int hardLevinshtein(Sequence root, Sequence query) {
		int arr[][] = new int[root.length+2][query.length+2];
		...
		for(int i = 2; i < root.length + 2; i++) {
			for(int j = 2; j < query.length + 2; j++) {
				...
				if(query.data.get(j-2).distance(root.data.get(i-2)) == 0)
					diff = 1;
				arr[i][j] = (int)min((arr[i-1][j] + 1), (arr[i][j-1] + 1), (arr[i-1][j-1] + diff));
			}
		}
	...
	public static int hardLevinshtein(int[] root, int[] query) {
		int arr[][] = new int[root.length+2][query.length+2];
	...
	public static void main(String[] args) {
		while(true) {
			...
			System.out.println(softLevinshtein(s1, s2, 2));
		}
	}
}